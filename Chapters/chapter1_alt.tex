...
The main processes for this rearrangement are two competing processes: radiative relaxation, and Auger electron emission.
...
In the case of radiative relaxation, an upper shell electron will move and occupy the hole left in the inner shell. During this process, energy is released in the form of photons with energy in the range of X-rays. The detection of these photons is what allows for an analysis and detection of the element at study.

On the other hand, an Auger emission occurs when in the process of an upper electron shifting to the inner shell hole, energy is released not in the form of photons, but by the atomic system going through the process of the ionization of a lesser bound electron.


In reality, when the initial vacancy is generated, two more processes can occur: shake-off and shake-up. Due to a sudden change in the potential felt by the remaining electrons, during the ionization process, one other electron may become unbound, leading to a second ionization, the shake-off process, or may be excited to an upper state, during shake-up. These processes occurrence probabilities are related to the rate at which the electron leaves the system during the first ionization. For lower electron emission energies, where the outgoing electron will spend much time in contact with the rest of the system during its leave, and there is not much energy for shake up/off to occur, the adiabatic regime reigns. In this regime, the shake probabilities are small, and share a proportionality with the beam's energy. For higher energies, the electrons' exit can be considered instantaneous, being in the presence of the sudden regime, where while the shake probabilities still increase with the energy of the beam, quickly saturate for a maximum value of probability by taking into account the electron's wavefunctions. This calculated if performed via the computation of overlaps between initial and final states, but is out of scope for this thesis. The model conceived in~\cite{Thomas_model} studies the shake probability as a function of energy.
...
The characteristic radiation measured from the radiative relaxation of a post-ionization unstable atomic system is one of the main ways of identifying an atomic element. This is due to the photons emitted from different elements possessing distinct, quantized values of energy, forming well-defined energy lines when observed in a spectrometer. These lines have specific notations which usually use the electron and the hole's subshell in order to identify what transition occurred.
...
As previously mentioned, throughout this thesis, while the study is focused on the characteristic radiation emitted during the atomic relaxation process, the main vacancy generation method at study will be due to an excitation (Figure~\ref{fig:exc}), instead of an ionization. This is one possibility of the many so-called satellite states, where the electronic configuration present during the relaxation process contains additional electrons or holes, or is simply not the standard ground state configuration after an ionization process.
The characteristic radiation from transition originated from these states may be one of the keys needed to fully comprehend and deconvolute an element's emission spectra.
...
The transition's radiation type can be of two main flavors, Electric, $E_k$, or Magnetic, $M_k$, with $k$ representing its multiplicity. These indicate which of the \gls{EMF} component had the strongest influence on the transition's occurrence.
For both these transitions the system's total angular momentum is allowed a change of $\Delta J = 0, \pm k$, and so does its projection, $M_J$. Different transitions lead to different changes of parity in the system and to different selection rules.
